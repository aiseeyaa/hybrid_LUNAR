...
def make_objective(train_x, val_x, val_y):
    def objective(trial):
        ...
        try:
            clf = OneClassSVM(kernel=kernel, nu=nu, gamma=gamma, degree=degree, coef0=coef0)
            clf.fit(train_x)
...
def fit_and_score_ocsvm(params, train_x, val_x, test_x):
    clf = OneClassSVM(
    ...
    scores_val = minmax_scale_scores(-clf.decision_function(val_x))

    start_inference = time.time()
    scores_test = minmax_scale_scores(-clf.decision_function(test_x))
...
def run_experiment(dataset, run_cfg, best_params):
    ...
    clf, scores_val, scores_test, runtime_train, runtime_inference = fit_and_score_ocsvm(
